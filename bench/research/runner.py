...
def load_research_corpus(path: str | Path) -> list[dict[str, Any]]:
    ...
    for i, line in enumerate(text.splitlines()):
        ...
        # expected_hits may live at top-level or inside answer object
...
def run_research(
    *,
    ...
    if not pool and router is None:
    ...
    # Systems: always_<model> for each pool model + aria_router if router set
    systems: list[tuple[str, str, EndpointConfig | None]] = []
    # (system_name, model_to_request, endpoint)
    for alias, cfg in pool.items():
        mid = alias_to_model[alias]
        systems.append((f"always_{mid}", mid, cfg))
    if router is not None:
        systems.append(("aria_router", entrypoint, router))
    else:
        notes.append("no --router; aria_router system omitted")

    results: list[dict[str, Any]] = []
    by_system: dict[str, list[dict[str, Any]]] = {name: [] for name, _, _ in systems}
    by_system_domain: dict[str, dict[str, list[dict[str, Any]]]] = {
        name: {} for name, _, _ in systems
    }
    ...
    for item in corpus:
        ...
        for sys_name, model, cfg in systems:
            if cfg is None:
                continue
            chat = fn(
            ...
            routed = chat.router_model if sys_name == "aria_router" else model
    ...
    for sys_name, _, _ in systems:
        ...
        summary = {
            ...
            "by_domain": {
                d: {
                    "mean_score": sum(float(r["score"]) for r in rs) / len(rs) if rs else 0.0,
                    "axes": mean_axes(rs),
                    "n": len(rs),
                }
    ...
    # Deltas vs best always baseline
    ...
    return {
        ...
        "meta": {
            ...
            "tasks": len(corpus),
            "systems": [n for n, _, _ in systems],
        },
        "summary": {
            "tasks": len(corpus),
