"""CLI: ``python -m bench routing|research|list-corpus|download-draco``."""
...
# Hugging Face datasets CDN for perplexity-ai/draco test split
_DRACO_URLS = (
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m bench",
        description=(
            "Router bench: ADR-040 routing ladder + Perplexity DRACO research "
    ...
    def add_common(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument("--router", default=None, help="aria-router base URL")
        sp.add_argument(
            "--entrypoint",
            default="aria/semantic-auto",
            help="virtual model name for router requests",
        )
        sp.add_argument(
            "--quality",
        ...
        sp.add_argument(
            ...
            default=[],
            metavar="ALIAS=KEY",
            help="bearer per pool alias or 'router'/'judge'",
        )
    ...
    pd.add_argument(
        "--out",
        default="draco_test.jsonl",
        help="output path (default: draco_test.jsonl)",
    )
...
def _download_draco(out: Path, timeout: float) -> int:
    last_err: str | None = None
    for url in _DRACO_URLS:
        try:
            ...
            continue
    print(
        f"skip: could not download DRACO test.jsonl ({last_err}); "
        "see bench/corpus/README.md",
        file=sys.stderr,
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.cmd == "download-draco":
            return _download_draco(Path(args.out), args.timeout)
        ...
        router_cfg = None
        if args.router:
            router_cfg = EndpointConfig(
                base_url=args.router,
                api_key=keys.get("router", ""),
                timeout_s=args.timeout,
            )

        judge_key = args.judge_api_key or keys.get("judge", "")
        ...
        if args.cmd == "routing":
            ...
            if not pool and not router_cfg:
